# code adapted from: https://github.com/molly-williams/deltaSLR_map

# user interface

library(shiny)
library(shinyjs)
library(dplyr)
library(sf)
library(leaflet)

# load all business (non-reactive) and server (reactive) logic

source("business-logic/fct_wrangling.R")
#source("server-logic/mod_01_extent_ui.R")
source("server-logic/mod_01_extent_server.R")

# navigation panel

ui <- navbarPage(
  title = 'Blue Forests',
  tabPanel('Explore Blue Forest Distributions',
           div(class="outer",
               tags$head(
                 includeCSS("styles.css")
               ),
               
               leafletOutput("map", width="100%", height="100%"),
               
               tags$style(".leaflet-control-layers-overlays{color: blue}"),
               
               absolutePanel(id = "controls", 
                             class = "panel panel-default", 
                             fixed = TRUE,
                             draggable = TRUE, 
                             top = 100, 
                             left = 30, 
                             right = "auto", 
                             bottom = "auto",
                             width = 330, 
                             height = "auto",
                             
                             tags$br(),
                             
                             tags$em("Allow a moment for layers to load."),
                             
                             selectInput("bf", 
                                         #label=NULL,
                                         label=h4(tags$b("1. Select blue forest:")), 
                                         choices = list("None" = 0, "Mangrove" = 1, "Seagrass" = 2, "Saltmarsh" = 3, "Kelp" = 4, 'All' = 5),
                                         selected = FALSE),
                             
                             tags$br(),
                             
                             selectInput("ct", label = h4(tags$b("2. Choose country or territory")), 
                                         choices =  terr, 
                                         selected = 'Global'),
                             
                             tags$br(),
                             
                             h4(tags$b("3. Show WWF Blue Forest projects")),
                             
                             checkboxInput("bfproj", label = NULL,
                                           value = FALSE)
                             
               ) #end absolute panel
      
           ), #end div
           
           tags$div(id="cite",
                    tags$em('This map was created in support of the Blue Forests Initiative, a project supported by WWF and the Global Wetlands Project'))
           
  ), # end tabpanel
) # end nav bar

# server logic

server <- function(input, output, session) {

  # map from server 
  mod_extent_server('map', forest = bf, cterr = ct, proj = bfproj)
  
} #end server

shinyApp(ui, server) 