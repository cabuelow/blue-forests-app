...
function(input, output, session) {
  ...
  observe({
    ...
    if(input$bfcheck != 'all'){  
      ...
      leafletProxy("map1") %>%
        clearControls() %>% 
        clearGroup(c('p1', 'p2', 'p3', 'p4', 'p5', 'p6', 'profile')) %>%
        clearMarkers() %>% 
        addPolygons(
          group = 'p1',
      ...
      leafletProxy("map1") %>%
        ...
        addLegend("bottomright",
                  colors = hot_pal, labels = c('Extent', 'Threat', 'Carbon', 'Cobenefit', 'Biodiversity'),
                  #title = "Critera colour",
                  opacity = 0.5)
    }}else{
      if(input$bfcheck != 'all'){  
        ...
        leafletProxy("map1") %>%
          ...
          addCircleMarkers(
            ...
            popup= my_popups) %>% 
          addLegend("bottomright",
                    pal = hot_pal, values = c('Extent', 'Threat', 'Carbon', 'Cobenefit', 'Biodiversity'),
                    #title = "Critera colour",
                    opacity = 1)
      }
    }}else{ # end wwf proj ifelse. start profile ifelse
      if(input$bfproj2 == FALSE){ # if-else to define whether wwf projects are on or off
      if(input$bfcheck != 'all'){  
