...
scores.pcrit <- scores %>% 
  pivot_longer(cols = c(mang_extent:kelp_extent, mang_threat:kelp_threat,
                        mang_carbon:kelp_carbon, mang_biodiversity:kelp_biodiversity,
                        mang_cobenefit:kelp_cobenefit), 
               names_to = 'forests', values_to = 'criteria_scores') %>% 
  mutate(forest = substr(forests, 1,4),
         criteria = substr(forests, 6, 20)) %>% 
  filter(criteria_scores != 0)
scores.pcrit$forest_name <- recode(scores.pcrit$forest, 'mang' = 'Mangrove', 'seag' = 'Seagrass', 'salt' = 'Saltmarsh', 'kelp' = 'Kelp')
